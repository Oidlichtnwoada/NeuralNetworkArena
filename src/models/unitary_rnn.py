...
class EUNNCell(tf.keras.layers.AbstractRNNCell):
    ...
    def fft_param(self, num_units, cplex):
        ...
        theta = self.add_weight("theta", [capacity, num_units // 2], initializer=phase_init, trainable=False)
        cos_theta = tf.cos(theta)
        sin_theta = tf.sin(theta)
        if cplex:
            phi = self.add_weight("phi", [capacity, num_units // 2], initializer=phase_init, trainable=False)
            cos_phi = tf.cos(phi)
            sin_phi = tf.sin(phi)
        ...
        if cplex:
            omega = self.add_weight("omega", [num_units], initializer=phase_init, trainable=False)
    ...
    def tunable_param(self, num_units, cplex, capacity):
        ...
        phase_init = tf.random_uniform_initializer(-3.14, 3.14)
        theta_A = self.add_weight("theta_A", [capacity_A, num_units // 2], initializer=phase_init, trainable=False)
        cos_theta_A = tf.cos(theta_A)
        sin_theta_A = tf.sin(theta_A)
        if cplex:
            phi_A = self.add_weight("phi_A", [capacity_A, num_units // 2], initializer=phase_init, trainable=False)
        ...
        theta_B = self.add_weight("theta_B", [capacity_B, num_units // 2 - 1], initializer=phase_init, trainable=False)
        cos_theta_B = tf.cos(theta_B)
        sin_theta_B = tf.sin(theta_B)
        if cplex:
            phi_B = self.add_weight("phi_B", [capacity_B, num_units // 2 - 1], initializer=phase_init, trainable=False)
