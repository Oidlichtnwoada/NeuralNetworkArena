...
%\todo{Enter your text here.}
    ...
    The inhibitorizy synapse that connects a neuron with the other neuron is responsible for suppressing the other neuron when a neuron itself has high potential. Therefore it ensures that the second neuron's potential is kept low such that only one neuron can have a high potential.
    ...
    The communication synapse would be connected from one neuron to the other neuron and must fulfill the recurrent and inhibitory synapse tasks.
    ...
    The benchmark has a configurable memory high symbol, memory low symbol, memory length, amount of cell switches, and amount of samples generated at each benchmark invocation.
    ...
    To achieve transparency on how the results were obtained, all logs generated during the three benchmark runs by the benchmark framework described in \Cref{benchmark_framework} are available under \url{https://github.com/Oidlichtnwoada/NeuralNetworkArena/tree/master/benchmark_logs}.
    ...
    It should be visible that the unitary matrix parameterization with the matrix exponential is more efficient than the approach with rotational matrices.
    ...
    Always the same models as reported in \Cref{activity_results} take a significant amount of time to train. Therefore, only exceptions to the norm in terms of training duration will be reported in the following.
    ...
    This benchmark task seemed to be challenging for the Transformer architecture, as maybe the positional encoding cannot be appropriately incorporated when attention to several individual positions is required.
    ...
    All of these models were able to solve the Memory Benchmark in some benchmark runs and failed in other runs, leading to a high test loss standard deviation of all these models.
    Maybe an improved initialization strategy for these models helps with this problem.
    Surprisingly, the Transformer architecture could not solve the Memory Benchmark in every single run, though it just needs to attend to the first input vector that contains the required category.
    ...
    As the Unitary RNN has a bounded loss gradient and performs exceptionally well on this benchmark, the Transformer derivative model that uses it performed well, too.
    ...
    All of these models could not solve the benchmark task as defined, but they all delivered decent results.
    ...
    Therefore, there is lots of future work to do to speed up this model's training by using simplifications or approximations in the model function.
    ...
    The GRU architecture outperformed the LSTM architecture by tradining in model complexity for hidden state size in all benchmarks.
    ...
    Furthermore, there were some other research topics regarding the Memory Cell discussed in \Cref{cell_results}.
