...
%\todo{Enter your text here.}
    ...
    The function the LSTM model is applying to its inputs to produce the outputs is given as follows with inputs denoted as $x_t$ and outputs which equals the hidden states denoted as $h_t$:
    \begin{align}
    ...
    The structure of the model allows it to capture long-term dependencies as by setting $f_t$ equal to one and $i_t$ equal to zero in some common vector indices, only the previous cell state is used to build the next cell state in these next cell state vector entries.
