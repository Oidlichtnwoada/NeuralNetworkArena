...
%\end{danksagung*}
    ...
    \begin{abstract}
        ...
        These benchmarks try to determine the model's capabilities to capture long-term dependencies and their abilities to model physical systems.
...
% Switch to Arabic numbering and start the enumeration of chapters in the table of content.
    ...
    The first discrete-time RNN was the Elman network proposed in \cite{elman_network} with a structure similar to the vanilla RNN architecture described in \Cref{long_term_difficult}.
    ...
    This procedure is especially used in encoder-decoder RNN architectures used for example in machine translation.
    ...
    Furthermore, bidirectional RNNs \cite{bidirectional_rnn} were introduced that are trained in positive and negative time direction simultaneously to partially mitigate the issue of learning long-term dependencies in RNNs.
    ...
    Then \cite{maximum_entropy_training} sped up the training of maximum entropy models by changing the form of the models to use classes.
    ...
    Moreover, \cite{multidimensional_rnn} introduced multidimensional RNNs that are able to handle input data with more than one spatio-temporal dimension.
    This model was successfully used in \cite{offline_handwriting} for the transcription of images of handwritten text.
    An Elman network-based language model was successfully applied to speech recognition in \cite{language_model_rnn}.
    The rectified linear unit activation function was introduced in \cite{relu} which helps dealing with the vanishing gradient problem in RNNs.
    ...
    Noise-contrastive estimation \cite{NCE} proposes a new objective function as alternative to the hierarchical softmax for probabilistic models which performs nonlinear logistic regression to discriminate between the observed data and artificially generated noise.
    ...
    Furthermore, \cite{SCRN} proposes a structurally constrained recurrent network (SCRN) whose recurrent weight matrix is partially close to the identity matrix which helps backpropagation.
    ...
    This kind of unitary RNNs were augmented with a mechanism to forget in \cite{unitary_rnn_forget}.
    The Neural Turing Machine architecture got further improved in \cite{DNC} to the Differentiable Neural Computer (DNC) which added a memory use link matrix and safe and efficient memory management.
    A GRU-based RNN with multidimensional and exponentially decaying state called CT-GRU was proposed in \cite{CTGRU}.
