...
\setdate{11}{03}{2021} % Set date with 3 arguments: {day}{month}{year}.
...
\newcommand{\benchmarkplots}[2]{
    ...
    The training plots for the #2 Benchmark and each model will be shown on the following pages.
    \clearpage{}
...
%\end{danksagung*}
    ...
    \begin{abstract}
        ...
        These benchmarks determine the model's capabilities to capture long-term dependencies and their abilities to model physical systems.
...
%\todo{Enter your text here.}
    ...
    Moreover, two discrete-time architectures with bounded loss gradients are introduced, given as the Unitary RNN introduced in \Cref{urnn} and the Matrix Exponential Unitary RNN introduced in \Cref{meurnn}.
    In \Cref{transformer} the Transformer architecture is introduced, which employs an encoder-decoder structure and uses an attention mechanism to capture long-term dependencies.
    ...
    As this thesis only uses regularly sampled time series, the continuous-time model is continually fed with the time input $1$ as mentioned in \Cref{ctrnn}.
    ...
    In this chapter, the used benchmark hardware is first specified in \Cref{benchmark_hardware}, which also includes the experiments' overall structure.
