...
def learned_init(units):
    return tf.squeeze(tf.keras.layers.Dense(units)(tf.ones([1, 1])))
...
class NTMCell(tf.keras.layers.Layer):
    ...
    def __call__(self, x, prev_state):
        ...
        with tf.compat.v1.variable_scope("o2p", reuse=(self.step > 0) or self.reuse):
            parameters = tf.keras.layers.Dense(total_parameter_num, kernel_initializer=self.o2p_initializer)(controller_output)
        ...
        with tf.compat.v1.variable_scope("o2o", reuse=(self.step > 0) or self.reuse):
            NTM_output = tf.keras.layers.Dense(output_dim, kernel_initializer=self.o2o_initializer)(tf.concat([controller_output] + read_vector_list, axis=1))
