...
def fft_param(num_units, cplex):
    ...
    if cplex:
        phi = tf.compat.v1.get_variable("phi", [capacity, num_units // 2], initializer=phase_init)
        cos_phi = tf.cos(phi)
        sin_phi = tf.sin(phi)
    ...
    if cplex:
        omega = tf.compat.v1.get_variable("omega", [num_units], initializer=phase_init)
...
def tunable_param(num_units, cplex, capacity):
    ...
    phase_init = tf.random_uniform_initializer(-3.14, 3.14)
    theta_A = tf.compat.v1.get_variable("theta_A", [capacity_A, num_units // 2], initializer=phase_init)
    cos_theta_A = tf.cos(theta_A)
    sin_theta_A = tf.sin(theta_A)
    if cplex:
        phi_A = tf.compat.v1.get_variable("phi_A", [capacity_A, num_units // 2], initializer=phase_init)
    ...
    theta_B = tf.compat.v1.get_variable("theta_B", [capacity_B, num_units // 2 - 1], initializer=phase_init)
    cos_theta_B = tf.cos(theta_B)
    sin_theta_B = tf.sin(theta_B)
    if cplex:
        phi_B = tf.compat.v1.get_variable("phi_B", [capacity_B, num_units // 2 - 1], initializer=phase_init)
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.compat.v1.variable_scope(scope or "eunn_cell"):
            ...
            if self._cplex:
                ...
                U = tf.compat.v1.get_variable("U", [inputs_size, self._num_units], initializer=input_matrix_init)
                inputs = tf.matmul(inputs, U)
            # hidden to hidden
            state = self.loop(state)
            # activation
            bias = tf.compat.v1.get_variable("modReLUBias", [self._num_units], initializer=tf.constant_initializer())
