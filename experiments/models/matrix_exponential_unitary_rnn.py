...
class MatrixExponentialUnitaryRNN(tf.keras.layers.AbstractRNNCell):
    def __init__(self, state_size, output_size, use_fft=False, trainable_initial_state=False, **kwargs):
        ...
        state_vector_initializer = tf.random_uniform_initializer(-math.pi, math.pi)
        self.real_state_vector = self.add_weight('real_state_vector', (self.state_size * (self.state_size + 1) // 2,), tf.float32, state_vector_initializer)
        self.imag_state_vector = self.add_weight('imag_state_vector', (self.state_size * (self.state_size + 1) // 2,), tf.float32, state_vector_initializer)
    ...
    def build(self, input_shape):
        ...
        input_matrix_initializer = tf.random_uniform_initializer(-1E-2, 1E-2)
        self.real_input_matrix = self.add_weight('real_input_matrix', (self.state_size, factor * inputs_size), tf.float32, input_matrix_initializer)
        self.imag_input_matrix = self.add_weight('imag_input_matrix', (self.state_size, factor * inputs_size), tf.float32, input_matrix_initializer)
